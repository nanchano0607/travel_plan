...
public class PlanController {
    ...
    public ResponseEntity<ApiResponse<SavePlanResponseDto>> savePlan(Authentication authentication, @Valid @RequestBody SavePlanDto requestDto) {
        requestDto.setUserId((Long)authentication.getPrincipal());
    ...
    public ResponseEntity<ApiResponse<SavePlanDto>> createAiPlanDraft(Authentication authentication, @Valid @RequestBody PlanRequestDto requestDto) {
        Long userId = (Long) authentication.getPrincipal();
        SavePlanDto response = chatService.createPlanDraft(userId, requestDto);
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "요청 값 오류 또는 AI 응답 파싱 실패")
    public ResponseEntity<ApiResponse<SavePlanDto>> retryAiPlanDraft(Authentication authentication, @Valid @RequestBody RetryPlanRequestDto requestDto) {
        Long userId = (Long) authentication.getPrincipal();
        SavePlanDto response = chatService.retryPlanDraft(userId, requestDto);
        return ResponseEntity.ok(ApiResponse.success(response));
    }
    ...
    public ResponseEntity<ApiResponse<List<SavePlanResponseDto>>> getPlansByUserId(Authentication authentication) {
        Long userId = (Long)authentication.getPrincipal();
